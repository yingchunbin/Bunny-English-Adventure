...
import { Package, X, Sprout, Bird, Factory, Armchair, ShoppingBasket, ArrowRightCircle, Ban, Plus, Zap, Coins, Clock, Shield, Layers } from 'lucide-react';
...
interface InventoryModalProps {
  ...
  onPlantAll?: (seedId: string) => void; // New Prop
...
const InventoryModalComponent: React.FC<InventoryModalProps> = ({ 
    ...
    mode, onSelectSeed, onPlantAll, onSelectAnimal, onSelectMachine, onToggleDecor, onClose, onGoToShop 
}) => {
                  ...
                  {mode === 'SELECT_SEED' && activeTab === 'SEEDS' && (
                      <div className="flex flex-col gap-1 items-end">
                          <button 
                              ...
                              className="bg-green-500 text-white px-4 py-2 rounded-xl font-black text-xs uppercase shadow-md active:scale-95 transition-all w-24"
                          >
                              Gieo hạt
                          </button>
                          {count > 1 && onPlantAll && (
                              <button 
                                  onClick={() => onPlantAll(item.id)}
                                  className="bg-amber-500 text-white px-4 py-2 rounded-xl font-black text-[10px] uppercase shadow-md active:scale-95 transition-all w-24 flex items-center justify-center gap-1"
                              >
                                  <Layers size={10} /> Gieo hết
                              </button>
                          )}
                      </div>
                  )}
