  ...
  return (
    <div className={`flex flex-col h-full animate-fadeIn relative overflow-hidden transition-colors duration-1000 ${userState.weather === 'RAINY' ? 'bg-slate-500' : 'bg-[#e0f2fe]'}`}>
      ...
      {flyingItems.map(item => (
          <div key={item.id} className={`fixed pointer-events-none font-black text-4xl z-[110] animate-floatUp flex items-center gap-2 ${item.type === 'COIN' ? 'text-amber-400' : item.type === 'HEART' ? 'text-red-500' : 'text-slate-800'}`} style={{ left: item.x, top: item.y }}>
              {item.type === 'COIN' ? <>{item.icon} <Coins size={32} fill="currentColor"/></> : item.icon}
          </div>
      ))}

      {activeModal === 'ORDERS' && <OrderBoard orders={userState.activeOrders || []} items={[...CROPS, ...PRODUCTS]} inventory={userState.harvestedCrops || {}} onDeliver={deliverOrder} onRefresh={triggerOrderRefreshQuiz} onClose={() => setActiveModal('NONE')} />}
      {activeModal === 'SHOP' && <ShopModal crops={CROPS} decorations={DECORATIONS} userState={userState} onBuySeed={(crop, amount) => { playSFX('success'); onUpdateState(prev => ({...prev, coins: prev.coins - crop.cost * amount, inventory: {...prev.inventory, [crop.id]: (prev.inventory[crop.id]||0) + amount}})); showToast(`Mua ${amount} hạt giống ${crop.name} thành công!`, "success"); }} onBuyDecor={(decor) => { onUpdateState(prev => ({...prev, decorations: [...(prev.decorations||[]), decor.id], coins: decor.currency === 'COIN' ? prev.coins - decor.cost : prev.coins})); playSFX('success'); showToast(`Đã mua vật phẩm trang trí!`, "success"); }} onClose={() => setActiveModal('NONE')} />}
      {activeModal === 'BARN' && (
          <BarnModal 
            ...
            onSell={(id) => {
                const item = [...CROPS, ...PRODUCTS].find(c => c.id === id);
                if (item) {
                    onUpdateState(prev => {
                        const currentCount = prev.harvestedCrops?.[id] || 0;
                        if (currentCount <= 0) return prev;
                        return {
                            ...prev, 
                            coins: prev.coins + item.sellPrice, 
                            harvestedCrops: {...prev.harvestedCrops, [id]: currentCount - 1}
                        };
                    });
                    playSFX('success');
                }
                ...
                if (item) {
                    onUpdateState(prev => {
                        ...
                        return {
                            ...prev, 
                            coins: prev.coins + item.sellPrice * currentCount, 
                            harvestedCrops: {...prev.harvestedCrops, [id]: 0}
                        };
                    });
                    playSFX('success');
                }
      ...
      {activeModal === 'WELL' && <WellModal words={allWords} onSuccess={(amount) => { addReward('WATER', amount); setActiveModal('NONE'); showToast(`Nhận ${amount} giọt nước từ Giếng Thần!`, "success"); }} onClose={() => setActiveModal('NONE')} />}
      {activeModal === 'PLOT' && selectedPlotId !== null && <PlotModal plot={userState.farmPlots.find(p => p.id === selectedPlotId)!} inventory={userState.inventory} waterDrops={userState.waterDrops} fertilizers={userState.fertilizers} userCoins={userState.coins} weather={userState.weather} nextPlotCost={nextPlotCost} onAction={handlePlotAction} onClose={() => setActiveModal('NONE')} />}
      {activeModal === 'MISSIONS' && <MissionModal missions={userState.missions || []} onClaim={(mission) => { onUpdateState(prev => ({ ...prev, missions: prev.missions?.map(m => m.id === mission.id ? { ...m, claimed: true } : m) })); addReward(mission.reward.type, mission.reward.amount); showToast(`Nhận thưởng: ${mission.reward.amount} ${mission.reward.type === 'COIN' ? 'đồng xu' : mission.reward.type}!`, "success"); }} onClose={() => setActiveModal('NONE')} />}
      ...
      {activeModal === 'ANIMAL_BUY' && selectedSlotId && (
          <AnimalShopModal 
            ...
            crops={CROPS} 
            products={PRODUCTS}
            userLevel={userState.completedLevels?.length || 0} 
            userCoins={userState.coins}
            onBuy={(animal) => {
                const res = buyAnimal(selectedSlotId, animal.id);
                if(res.success) { setActiveModal('NONE'); showToast("Mua thành công!", "success"); } 
                else showToast(res.msg || "Lỗi", "error");
            }}
            onClose={() => setActiveModal('NONE')}
          />
      ...
      {activeModal === 'MACHINE_BUY' && selectedMachineSlotId && (
          <MachineShopModal 
            ...
            userLevel={userState.completedLevels?.length || 0}
            userCoins={userState.coins}
            onBuy={(machine) => buyMachine(machine)}
            onClose={() => setActiveModal('NONE')}
          />
      )}
      ...
      {activeModal === 'MACHINE_CRAFT' && selectedMachineSlotId && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4 backdrop-blur-md animate-fadeIn">
              <div className="bg-white rounded-[2.5rem] w-full max-w-sm p-6 relative border-4 border-blue-200">
                  <button onClick={() => setActiveModal('NONE')} className="absolute top-4 right-4"><X /></button>
                  <h3 className="text-xl font-black text-center text-blue-700 mb-6 uppercase">Chế Biến</h3>
                  <div className="grid grid-cols-1 gap-4 max-h-[60vh] overflow-y-auto no-scrollbar">
                      {(() => {
                          const slot = userState.machineSlots?.find(s => s.id === selectedMachineSlotId);
                          const machineRecipes = RECIPES.filter(r => r.machineId === slot?.machineId);
                          
                          if (machineRecipes.length === 0) return <div className="text-center text-slate-400">Không có công thức nào.</div>;

                          return machineRecipes.map(recipe => {
                              const outputItem = PRODUCTS.find(p => p.id === recipe.outputId);
                              const canCraft = recipe.input.every(req => (userState.harvestedCrops?.[req.id] || 0) >= req.amount);

                              return (
                                  <button key={recipe.id} onClick={() => startCrafting(recipe)} className={`flex flex-col bg-blue-50 p-4 rounded-2xl border-2 transition-all active:scale-95 ${canCraft ? 'hover:bg-blue-100 border-blue-200' : 'opacity-70 border-slate-200'}`}>
                                      <div className="flex items-center justify-between w-full mb-3">
                                          <div className="flex items-center gap-3">
                                              <span className="text-4xl drop-shadow-sm">{outputItem?.emoji}</span>
                                              <div className="text-left">
                                                  <div className="font-black text-slate-800">{recipe.name}</div>
                                                  <div className="text-[10px] text-slate-500 font-bold flex items-center gap-1"><Timer size={10}/> {Math.ceil(recipe.duration/60)} phút</div>
                                              </div>
                                          </div>
                                          {!canCraft && <div className="text-[10px] text-red-500 font-black bg-white px-2 py-1 rounded-lg border border-red-100">Thiếu đồ</div>}
                                      </div>
                                      
                                      <div className="bg-white p-2 rounded-xl flex items-center gap-2 flex-wrap border border-blue-100">
                                          {recipe.input.map(req => {
                                              const ingredient = [...CROPS, ...PRODUCTS].find(x => x.id === req.id);
                                              const owned = userState.harvestedCrops?.[req.id] || 0;
                                              const isEnough = owned >= req.amount;
                                              return (
                                                  <div key={req.id} className={`flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-lg border ${isEnough ? 'bg-green-50 text-green-700 border-green-100' : 'bg-red-50 text-red-700 border-red-100'}`}>
                                                      <span>{ingredient?.emoji}</span> {owned}/{req.amount}
                                                  </div>
                                              )
                                          })}
                                      </div>
      ...
      {activeModal === 'PET_INTERACT' && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4 backdrop-blur-xl animate-fadeIn">
              <div className="bg-white rounded-[4rem] w-full max-sm border-8 border-rose-200 overflow-hidden shadow-2xl flex flex-col">
                  <div className="bg-gradient-to-r from-rose-500 to-pink-500 p-8 flex justify-between items-center text-white">
                      <h3 className="text-2xl font-black flex items-center gap-3 uppercase"><Heart size={32} fill="white"/> NHÀ CỦA BÉ</h3>
                      <button onClick={() => setActiveModal('NONE')} className="p-2 hover:bg-white/20 rounded-full transition-colors"><X size={28}/></button>
                  </div>
                  <div className="p-8 flex flex-col items-center bg-rose-50/30">
                      <div className="relative mb-8 scale-150 transform hover:rotate-12 transition-transform">
                          <Avatar emoji={currentAvatar?.emoji || '🐰'} bgGradient={currentAvatar?.bgGradient || ''} size="md" animate />
                      </div>
                      <div className="w-full bg-white p-6 rounded-[2.5rem] mb-8 border-4 border-rose-100 shadow-inner text-center">
                          <div className="flex justify-between text-[11px] font-black text-slate-400 mb-3 uppercase tracking-widest">
                              <span>Vui vẻ: {userState.petHappiness || 0}/100</span>
                              <span className="text-rose-600">Cấp {userState.petLevel || 1}</span>
                          </div>
                          <div className="w-full h-6 bg-slate-100 rounded-full overflow-hidden relative shadow-inner">
                              <div className="h-full bg-gradient-to-r from-rose-400 to-pink-500 transition-all duration-1000" style={{ width: `${userState.petHappiness || 0}%` }} />
                          </div>
                          <p className="text-xs text-rose-400 mt-2 font-bold italic">"Cho tớ ăn nông sản nhé!"</p>
                      </div>
                      <div className="grid grid-cols-3 gap-2 w-full">
                          {CROPS.filter(c => !c.isMagic && (userState.harvestedCrops?.[c.id] || 0) > 0).slice(0, 3).map(c => (
                              <button key={c.id} onClick={(e) => { const rect = e.currentTarget.getBoundingClientRect(); const res = feedPet(c.id); if (res.success) { showToast(res.msg || "Măm măm ngon quá!", "success"); spawnFlyingItem(rect, "❤️", 'HEART'); } else showToast(res.msg || "Lỗi", "error"); }} className="bg-white p-2 rounded-2xl border-2 border-pink-100 flex flex-col items-center hover:bg-pink-50 active:scale-90 transition-all">
                                  <span className="text-2xl">{c.emoji}</span>
                                  <span className="text-[10px] font-black uppercase text-pink-600">x{userState.harvestedCrops?.[c.id]}</span>
                              </button>
                          ))}
                      </div>
                      {(!userState.harvestedCrops || Object.values(userState.harvestedCrops).every(v => !v)) && (
                          <div className="text-center text-slate-400 text-xs mt-4">Hết đồ ăn rồi! Bé hãy trồng thêm nhé.</div>
                      )}
                  </div>
              </div>
          </div>
      ...
      {activeModal === 'QUIZ' && quizContext && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/90 p-4 backdrop-blur-2xl animate-fadeIn">
              <div className="bg-white rounded-[4rem] w-full max-sm shadow-2xl border-8 border-indigo-400 overflow-hidden flex flex-col h-auto max-h-[90vh]">
                  ...
                  <div className="p-4 flex-1 overflow-y-auto relative no-scrollbar">
                      {quizContext.type === 'FLASHCARD' && <FlashcardGame words={quizContext.words} initialMode="QUIZ" onComplete={handleQuizSuccess} />}
